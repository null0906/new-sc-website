...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
    ...
    .pb-free{background:rgb(var(--green-rgb) / .15);color:var(--green)}
    .pb-paid{background:rgb(var(--accent-rgb) / .15);color:var(--accent)}
    ...
    .tmpl-rate{font-size:.72rem;font-weight:700;color:var(--red);background:rgb(var(--red-rgb) / .1);padding:.1rem .5rem;border-radius:4px;white-space:nowrap;margin-left:.4rem}
        ...
        <div className="img-wrap">
          <svg viewBox="0 0 960 400" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',display:'block',background:'linear-gradient(160deg,#071728 0%,#040f1b 100%)'}}>
            <defs>
              ...
              <linearGradient id="pg2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--green)"/><stop offset="100%" stopColor="var(--green)"/></linearGradient>
              <linearGradient id="pg3" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--red)"/><stop offset="100%" stopColor="var(--red)"/></linearGradient>
            ...
            {/* LEFT: 7-Step process */}
            <rect x="16" y="52" width="210" height="332" rx="12" fill="var(--bg-secondary)" stroke="rgb(var(--accent-rgb) / 0.15)" strokeWidth="1"/>
            ...
            {[
              ...
              <g key={i}>
                ...
                <circle cx="47" cy={100+i*42} r="10" fill={done?"rgb(var(--green-rgb) / 0.12)":"rgb(var(--accent-rgb) / 0.08)"} stroke={done?"var(--green)":"rgb(var(--accent-rgb) / 0.3)"} strokeWidth="1.2"/>
                <text x="47" y={104+i*42} textAnchor="middle" fill={done?"var(--green)":"rgb(var(--accent-rgb) / 0.5)"} fontSize="8" fontWeight="700" fontFamily="sans-serif">{done?"✓":`${i+1}`}</text>
                <text x="66" y={103+i*42} fill={done?"rgba(200,214,229,0.8)":"rgba(200,214,229,0.45)"} fontSize="9" fontFamily="sans-serif">{step}</text>
              </g>
            ))}

            <line x1="240" y1="48" x2="240" y2="388" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>
            ...
            {(() => {
              ...
              return (
                <>
                  <path d={area} fill="rgb(var(--accent-rgb) / 0.06)"/>
                  <path d={path} fill="none" stroke="url(#pg1)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                  {pts.map(([v],i)=>(
                    <circle key={i} cx={xs[i]} cy={ys[i]} r="3.5" fill="var(--accent)" stroke="var(--bg-primary)" strokeWidth="1.5"/>
                  ))}
                </>
              );
            })()}
            ...
            <text x="740" y="98" fill="var(--green)" fontSize="9" fontWeight="700" fontFamily="sans-serif">4% ↓</text>
            <text x="284" y="95" fill="var(--red)" fontSize="9" fontWeight="700" fontFamily="sans-serif">38%</text>

            {/* Metric cards */}
            {[
              {label:'Baseline Click Rate',  val:'38%', col:'var(--red)', x:268},
              {label:'After 12 Months',      val:'4%',  col:'var(--green)', x:368},
              {label:'Report Rate',          val:'61%', col:'var(--purple)', x:468},
              {label:'Reduction',            val:'89%', col:'var(--accent)', x:568},
            ].map(({label,val,col,x},i)=>(
              <g key={i}>
                <rect x={x} y={230} width="82" height="52" rx="9" fill="var(--bg-secondary)" stroke={`${col}28`} strokeWidth="1"/>
            ...
            {[
              {dept:'Finance',    rate:'42%', col:'var(--red)', w:168},
              {dept:'Operations', rate:'31%', col:'var(--yellow)', w:124},
              {dept:'Engineering',rate:'18%', col:'var(--accent)', w:72},
              {dept:'Leadership', rate:'28%', col:'var(--yellow)', w:112},
            ...
            <line x1="660" y1="48" x2="660" y2="388" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>
            ...
            {[
              {tmpl:'IT Password Reset',        rate:'41%', c:'var(--red)'},
              {tmpl:'HR Policy Acknowledgment', rate:'38%', c:'var(--red)'},
              {tmpl:'Invoice Approval Request', rate:'34%', c:'var(--yellow)'},
              {tmpl:'Delivery Tracking Link',   rate:'29%', c:'var(--yellow)'},
              {tmpl:'Google Drive Share',       rate:'26%', c:'var(--accent)'},
              {tmpl:'CEO Wire Transfer',        rate:'22%', c:'var(--accent)'},
            ].map(({tmpl,rate,c},i)=>(
              <g key={i}>
                <rect x="670" y={84+i*50} width="274" height="38" rx="8" fill="var(--bg-secondary)" stroke={`${c}22`} strokeWidth="1"/>
