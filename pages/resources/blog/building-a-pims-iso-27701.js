...
`}</style>
      ...
      <div className="hero-wrap">
        ...
        <div className="img-wrap">
          ...
          <defs><pattern id="pgd" width="22" height="22" patternUnits="userSpaceOnUse"><circle cx="11" cy="11" r=".7" fill="rgb(var(--accent-rgb) / 0.04)"/></pattern></defs>
          ...
          {['Scope','Gap Assessment','Build RoPA','Risk Assessment','Implement Controls','Policies & Notices','Training & Audit','Certification'].map((label, i) => {
            const x = 30 + i * 116;
            const cols = ['var(--purple)','var(--accent)','var(--green)','var(--yellow)','var(--red)','var(--purple)','var(--accent)','var(--green)'];
            return (<g key={i}>
              <rect x={x} y="38" width="108" height="200" rx="10" fill="var(--bg-secondary)" stroke={cols[i]+'22'} strokeWidth="1.5"/>
