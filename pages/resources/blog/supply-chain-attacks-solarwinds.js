...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
    ...
    .timeline-wrap::before{content:'';position:absolute;left:7px;top:0;bottom:0;width:2px;background:linear-gradient(180deg,var(--accent),rgb(var(--accent-rgb) / 0.1))}
    ...
    @media(max-width:600px){.cspm-hero-h1{font-size:1.65rem}.toc-grid{grid-template-columns:1fr}}
      ...
      <div className="cspm-hero-wrap">
        ...
        <div className="img-wrap">
          <svg viewBox="0 0 960 400" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',display:'block',background:'linear-gradient(160deg,#071728 0%,#040f1b 100%)'}}>
            <defs>
              <linearGradient id="scg1" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--accent)"/><stop offset="100%" stopColor="var(--accent-light)"/></linearGradient>
              <linearGradient id="scg2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--red)"/><stop offset="100%" stopColor="var(--red)"/></linearGradient>
              <linearGradient id="scg3" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--green)"/><stop offset="100%" stopColor="var(--green)"/></linearGradient>
              <pattern id="scpat" width="22" height="22" patternUnits="userSpaceOnUse"><circle cx="11" cy="11" r=".7" fill="rgb(var(--accent-rgb) / 0.04)"/></pattern>
            </defs>
            ...
            {[
              {phase:'Initial Access',    detail:'Build pipeline compromise - Feb 2020', col:'var(--red)'},
              {phase:'Persistence',       detail:'SUNBURST backdoor injected into Orion DLL', col:'var(--red)'},
              {phase:'Delivery',          detail:'Malicious update signed, pushed to 18,000 orgs', col:'var(--accent)'},
              {phase:'C2 Activation',     detail:'2-week dormancy, then beacon to avsvmcloud.com', col:'var(--accent)'},
              {phase:'Lateral Movement',  detail:'Golden SAML, credential theft, pivoting', col:'var(--yellow)'},
              {phase:'Exfiltration',       detail:'Data from US Treasury, DHS, FireEye, MSFT', col:'var(--yellow)'},
              {phase:'Discovery',         detail:'FireEye detects anomalous OAuth token - Dec 2020', col:'var(--green)'},
            ].map(({phase,detail,col},i)=>(
              <g key={i}>
                <rect x="20" y={66+i*40} width="400" height="30" rx="7" fill="var(--bg-secondary)" stroke={`${col}25`} strokeWidth="1"/>
            ...
            <line x1="438" y1="46" x2="438" y2="388" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>

            {/* Centre: Impact */}
            <text x="590" y="56" fill="rgba(200,214,229,0.7)" fontSize="10" fontWeight="700" fontFamily="sans-serif">BREACH IMPACT</text>
            {[
              {label:'Organisations affected',     val:'18,000+',   col:'var(--red)'},
              {label:'Months undetected',          val:'9–14 mo',   col:'var(--red)'},
              {label:'US Federal agencies hit',    val:'9',         col:'var(--accent)'},
              {label:'Fortune 500 victims',        val:'425+',      col:'var(--accent)'},
              {label:'Attack attribution',         val:'Russia SVR',col:'var(--yellow)'},
              {label:'Remediation cost (est.)',    val:'$40B+',     col:'var(--yellow)'},
            ].map(({label,val,col},i)=>(
              <g key={i}>
                <rect x="450" y={66+i*46} width="270" height="36" rx="9" fill="var(--bg-secondary)" stroke={`${col}22`} strokeWidth="1"/>
            ...
            <line x1="740" y1="46" x2="740" y2="388" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>

            {/* Right: Controls */}
            <text x="758" y="56" fill="rgba(200,214,229,0.7)" fontSize="10" fontWeight="700" fontFamily="sans-serif">WHAT WOULD HAVE HELPED</text>
            {[
              {ctrl:'SBOM for Orion platform',          status:'Would detect', col:'var(--green)'},
              {ctrl:'Build pipeline integrity checks',   status:'Would prevent',col:'var(--green)'},
              {ctrl:'SLSA Level 3+ controls',            status:'Would prevent',col:'var(--green)'},
              {ctrl:'Network anomaly detection',          status:'Earlier detect',col:'var(--yellow)'},
              {ctrl:'Zero Trust architecture',            status:'Limited blast', col:'var(--yellow)'},
              {ctrl:'Vendor security assessment',         status:'Insufficient', col:'var(--red)'},
            ].map(({ctrl,status,col},i)=>(
              <g key={i}>
                <rect x="752" y={66+i*52} width="192" height="40" rx="8" fill="var(--bg-secondary)" stroke={`${col}22`} strokeWidth="1"/>
      ...
      <div className="blog-layout">
        <article className="art">
          ...
          <p style={{fontSize:'1.05rem',lineHeight:'1.85',color:'var(--tb)',marginBottom:'1.5rem',borderLeft:'3px solid rgb(var(--accent-rgb) / 0.3)',paddingLeft:'1.2rem'}}>For decades, the software security model was built on a simple premise: verify the vendor, verify the certificate, trust the update. SolarWinds shattered that model in a way that is still reshaping how organisations think about trust. This article breaks down exactly how the attack worked, why it succeeded despite the victims having mature security programmes, and the specific controls that every organisation must now have in place - informed by the single most instructive supply chain attack in cybersecurity history.</p>
