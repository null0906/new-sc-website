...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
    ...
    .role-ctrl{background:rgb(var(--purple-rgb) / .06);border:1px solid rgb(var(--purple-rgb) / .22)}
    .role-proc{background:rgb(var(--green-rgb) / .04);border:1px solid rgb(var(--green-rgb) / .2)}
    .role-card .rc-lbl{font-size:.68rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;margin-bottom:.5rem}
    .role-ctrl .rc-lbl{color:var(--purple)}
    .role-proc .rc-lbl{color:var(--green)}
    ...
    .role-ctrl ul li::before{content:'→';color:var(--purple);flex-shrink:0}
    .role-proc ul li::before{content:'→';color:var(--green);flex-shrink:0}
    ...
    .ri-badge{font-size:.62rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;padding:.15rem .55rem;border-radius:4px;background:rgb(var(--accent-rgb) / .1);color:var(--cy);white-space:nowrap;flex-shrink:0}
    ...
    .roadmap-list::before{content:'';position:absolute;left:7px;top:0;bottom:0;width:2px;background:linear-gradient(180deg,var(--accent),rgb(var(--accent-rgb) / .1))}
    ...
    .rm-duration{display:inline-block;background:rgb(var(--accent-rgb) / .1);color:var(--cy);font-size:.68rem;font-weight:700;padding:.1rem .5rem;border-radius:4px;margin-bottom:.3rem}
    ...
    .reg-table tr:hover td{background:rgb(var(--accent-rgb) / .03)}
      ...
      <div className="cspm-hero-wrap">
        ...
        <div className="img-wrap">
          <svg viewBox="0 0 960 400" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',display:'block',background:'linear-gradient(160deg,#071728 0%,#040f1b 100%)'}}>
            <defs>
              <linearGradient id="pg1" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--accent)"/><stop offset="100%" stopColor="var(--accent-light)"/></linearGradient>
              <linearGradient id="pg2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--purple)"/><stop offset="100%" stopColor="#6366f1"/></linearGradient>
              <pattern id="ppat" width="22" height="22" patternUnits="userSpaceOnUse"><circle cx="11" cy="11" r=".7" fill="rgb(var(--accent-rgb) / 0.04)"/></pattern>
            </defs>
            ...
            <rect x="20" y="62" width="290" height="316" rx="12" fill="rgb(var(--purple-rgb) / 0.05)" stroke="rgb(var(--purple-rgb) / 0.2)" strokeWidth="1.5" strokeDasharray="4,3"/>
            <text x="165" y="82" textAnchor="middle" fill="var(--purple)" fontSize="9" fontWeight="700" fontFamily="sans-serif">ISO 27001 - Information Security (ISMS)</text>
            {['Risk assessment & treatment','Access control & encryption','Incident management','Business continuity','Supplier security'].map((t,i)=>(
              <g key={i}><rect x="34" y={90+i*38} width="262" height="28" rx="7" fill="rgb(var(--purple-rgb) / 0.07)" stroke="rgb(var(--purple-rgb) / 0.15)" strokeWidth="1"/><text x="46" y={108+i*38} fill="rgba(200,214,229,0.65)" fontSize="8.5" fontFamily="sans-serif">{t}</text></g>
            ))}
            <rect x="20" y="286" width="290" height="88" rx="10" fill="rgb(var(--accent-rgb) / 0.07)" stroke="rgb(var(--accent-rgb) / 0.28)" strokeWidth="1.5"/>
            <text x="165" y="306" textAnchor="middle" fill="var(--accent)" fontSize="9" fontWeight="700" fontFamily="sans-serif">ISO 27701 Extension - PIMS</text>
            ...
            <line x1="328" y1="44" x2="328" y2="388" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>
            ...
            {[
              {clause:'Clause 5', title:'Leadership',             detail:'Privacy policy · Privacy Officer · senior commitment', col:'var(--purple)'},
              {clause:'Clause 6', title:'Planning',               detail:'Privacy risk assessment · RoPA · privacy objectives',   col:'var(--purple)'},
              {clause:'Clause 7', title:'Support',                detail:'PII staff training · PIMS documentation',               col:'var(--purple)'},
              {clause:'Clause 8', title:'Operations (Controller)',detail:'Legal basis · consent · data subject rights · DPIAs',   col:'var(--accent)'},
              {clause:'Clause 8', title:'Operations (Processor)', detail:'Controller instructions · sub-processor register',     col:'var(--green)'},
              {clause:'Clause 9', title:'Performance Evaluation', detail:'Internal PIMS audit · management review',              col:'var(--yellow)'},
              {clause:'Clause 10',title:'Improvement',            detail:'Corrective actions · continual improvement',           col:'var(--yellow)'},
            ].map(({clause,title,detail,col},i)=>(
              <g key={i}>
                <rect x="346" y={62+i*46} width="360" height="36" rx="8" fill="var(--bg-secondary)" stroke={`${col}22`} strokeWidth="1"/>
            ...
            <line x1="722" y1="44" x2="722" y2="388" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>
            ...
            {[
              {reg:'GDPR',      coverage:'Articles 5,6,7,13-17,25,28,30,32,33,35', col:'var(--purple)', pct:88},
              {reg:'DPDP Act',  coverage:'Sections 6,7,8,11 - consent, rights, safeguards', col:'var(--accent)', pct:79},
              {reg:'CCPA/CPRA', coverage:'Consumer rights - access, delete, portability', col:'var(--yellow)', pct:74},
            ].map(({reg,coverage,col,pct},i)=>(
              <g key={i}>
                <rect x="732" y={62+i*110} width="212" height="96" rx="11" fill="var(--bg-secondary)" stroke={`${col}25`} strokeWidth="1"/>
