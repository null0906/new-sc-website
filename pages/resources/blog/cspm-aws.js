...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
    ...
    .sev-crit{background:rgb(var(--red-rgb) / .15);color:var(--red)}
    .sev-high{background:rgb(var(--yellow-rgb) / .15);color:var(--yellow)}
    .sev-med{background:rgb(var(--accent-rgb) / .15);color:var(--accent)}
      ...
      <div className="cspm-hero-wrap">
        ...
        <div className="img-wrap">
          <svg viewBox="0 0 960 420" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',display:'block',background:'linear-gradient(160deg,#071728 0%,#040f1b 100%)'}}>
            <defs>
              <linearGradient id="g1" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--accent)"/><stop offset="100%" stopColor="var(--accent-light)"/></linearGradient>
              <linearGradient id="g2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--green)"/><stop offset="100%" stopColor="var(--green)"/></linearGradient>
              <linearGradient id="g3" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--red)"/><stop offset="100%" stopColor="var(--red)"/></linearGradient>
              <linearGradient id="g4" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--yellow)"/><stop offset="100%" stopColor="#e09500"/></linearGradient>
              <pattern id="dots" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="12" cy="12" r=".8" fill="rgb(var(--accent-rgb) / 0.04)"/></pattern>
            </defs>
            <rect width="960" height="420" fill="url(#dots)"/>

            {/* LEFT: AWS Service coverage */}
            <rect x="16" y="16" width="220" height="388" rx="12" fill="var(--bg-secondary)" stroke="rgb(var(--accent-rgb) / 0.15)" strokeWidth="1"/>
            <text x="36" y="44" fill="rgba(200,214,229,0.7)" fontSize="10" fontWeight="700" fontFamily="sans-serif">AWS SERVICES MONITORED</text>
            {[
              {svc:'S3 Buckets',         findings:12, col:'var(--red)', pct:92},
              {svc:'IAM Policies',       findings:8,  col:'var(--yellow)', pct:78},
              {svc:'EC2 / Security Groups',findings:6, col:'var(--yellow)', pct:65},
              {svc:'RDS Instances',      findings:3,  col:'var(--accent)', pct:44},
              {svc:'CloudTrail',         findings:2,  col:'var(--green)', pct:28},
              {svc:'VPC / Flow Logs',    findings:1,  col:'var(--green)', pct:18},
              {svc:'Lambda Functions',   findings:0,  col:'var(--green)', pct:0},
            ...
            <line x1="250" y1="12" x2="250" y2="408" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>
            ...
            <text x="490" y="166" textAnchor="middle" fill="rgb(var(--yellow-rgb) / 0.7)" fontSize="9" fontFamily="sans-serif">Needs Attention</text>
            ...
            {/* Severity breakdown */}
            {[
              {label:'Critical', count:'4',  col:'var(--red)', x:310},
              {label:'High',     count:'12', col:'var(--yellow)', x:410},
              {label:'Medium',   count:'11', col:'var(--accent)', x:510},
              {label:'Low',      count:'5',  col:'var(--green)', x:610},
            ].map(({label,count,col,x},i)=>(
              <g key={i}>
                <rect x={x} y={226} width="80" height="52" rx="9" fill="var(--bg-secondary)" stroke={`${col}30`} strokeWidth="1"/>
            ...
            <line x1="718" y1="12" x2="718" y2="408" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>

            {/* RIGHT: Top findings */}
            <text x="736" y="42" fill="rgba(200,214,229,0.7)" fontSize="10" fontWeight="700" fontFamily="sans-serif">TOP CRITICAL FINDINGS</text>
            {[
              {title:'S3 bucket publicly accessible', sev:'CRITICAL', c:'var(--red)'},
              {title:'Root account MFA disabled',     sev:'CRITICAL', c:'var(--red)'},
              {title:'Security group: 0.0.0.0/0 SSH', sev:'HIGH',     c:'var(--yellow)'},
              {title:'EBS volume unencrypted',         sev:'HIGH',     c:'var(--yellow)'},
              {title:'CloudTrail logging disabled',    sev:'HIGH',     c:'var(--yellow)'},
              {title:'IAM access key >90 days old',   sev:'MEDIUM',   c:'var(--accent)'},
            ].map(({title,sev,c},i)=>(
              <g key={i}>
                <rect x="728" y={58+i*56} width="216" height="44" rx="8" fill="var(--bg-secondary)" stroke={`${c}28`} strokeWidth="1"/>
