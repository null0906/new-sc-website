  ...
  return (
    <Layout>
      <Head>
        ...
        <link rel="canonical" href="https://seccomply.net/resources/blog/endpoint-security-hybrid-work" />        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        <div className="img-wrap">
          <svg viewBox="0 0 960 420" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',display:'block',background:'linear-gradient(160deg,#071728 0%,#040f1b 100%)'}}>
            <defs>
              ...
              <pattern id="ep" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="12" cy="12" r=".8" fill="rgb(var(--accent-rgb) / 0.04)"/></pattern>
            </defs>
            <rect width="960" height="420" fill="url(#ep)"/>
            {/* Device fleet panel */}
            <rect x="18" y="18" width="220" height="384" rx="12" fill="var(--bg-secondary)" stroke="rgb(var(--accent-rgb) / 0.15)" strokeWidth="1"/>
            ...
            {[
              {label:'Corp Laptops',     n:'124', status:'✓ All enrolled', c:'var(--green)', bar:118},
              {label:'BYOD / Personal',  n:'68',  status:'⚠ 31 unmanaged', c:'var(--yellow)', bar:64},
              {label:'Contractor Devices',n:'35', status:'✗ 29 no MDM',    c:'var(--red)', bar:33},
              {label:'Mobile Phones',    n:'20',  status:'⚠ 8 unmanaged',  c:'var(--yellow)', bar:19},
            ...
            <text x="480" y="170" textAnchor="middle" fill="rgb(var(--accent-rgb) / 0.7)" fontSize="9" fontFamily="sans-serif">Fleet Compliant</text>
            ...
            {/* Mini cards */}
            <rect x="360" y="236" width="84" height="54" rx="8" fill="var(--bg-secondary)" stroke="rgb(var(--green-rgb) / 0.2)" strokeWidth="1"/>
            <text x="402" y="258" textAnchor="middle" fill="var(--green)" fontSize="18" fontWeight="700" fontFamily="sans-serif">156</text>
            <text x="402" y="272" textAnchor="middle" fill="rgba(200,214,229,0.4)" fontSize="8" fontFamily="sans-serif">Encrypted</text>
            <text x="402" y="282" textAnchor="middle" fill="rgb(var(--green-rgb) / 0.5)" fontSize="7" fontFamily="sans-serif">✓ compliant</text>
            <rect x="456" y="236" width="84" height="54" rx="8" fill="var(--bg-secondary)" stroke="rgb(var(--red-rgb) / 0.2)" strokeWidth="1"/>
            <text x="498" y="258" textAnchor="middle" fill="var(--red)" fontSize="18" fontWeight="700" fontFamily="sans-serif">60</text>
            <text x="498" y="272" textAnchor="middle" fill="rgba(200,214,229,0.4)" fontSize="8" fontFamily="sans-serif">Unencrypted</text>
            <text x="498" y="282" textAnchor="middle" fill="rgb(var(--red-rgb) / 0.5)" fontSize="7" fontFamily="sans-serif">⚠ fix required</text>
            {/* Control checks */}
            ...
            <line x1="614" y1="14" x2="614" y2="406" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>
            {/* Right: 5 risks */}
            <text x="634" y="44" fill="rgba(200,214,229,0.7)" fontSize="10" fontWeight="700" fontFamily="sans-serif">TOP 5 HYBRID RISKS</text>
            {[
              {risk:'No MDM, Contractor/BYOD Devices', sev:'CRITICAL', c:'var(--red)'},
              {risk:'Stale OS on Remote Machines',       sev:'HIGH',     c:'var(--yellow)'},
              {risk:'Unencrypted Devices Off-Site',      sev:'HIGH',     c:'var(--yellow)'},
              {risk:'Home Network as Perimeter',         sev:'MEDIUM',   c:'var(--accent)'},
              {risk:'Zero Audit Trail for Evidence',     sev:'HIGH',     c:'var(--yellow)'},
            ].map(({risk,sev,c},i)=>(
              <g key={i}>
                <rect x="626" y={60+i*60} width="314" height="48" rx="8" fill="var(--bg-secondary)" stroke={`${c}30`} strokeWidth="1"/>
