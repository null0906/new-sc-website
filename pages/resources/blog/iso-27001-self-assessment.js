...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
    ...
    .trigger-header{display:flex;align-items:center;gap:.8rem;margin:3rem 0 1rem;padding:.6rem 1.2rem;background:rgb(var(--green-rgb) / .05);border:1px solid rgb(var(--green-rgb) / .2);border-radius:10px}
    ...
    .sc-1{background:rgb(var(--yellow-rgb) / .05);border:1px solid rgb(var(--yellow-rgb) / .2)}
    .sc-2{background:rgb(var(--accent-rgb) / .06);border:1px solid rgb(var(--accent-rgb) / .2)}
    .sc-3{background:rgb(var(--red-rgb) / .06);border:1px solid rgb(var(--red-rgb) / .2)}
    ...
    .sc-1 .sc-time{background:rgb(var(--yellow-rgb) / .12);color:var(--am)}
    .sc-2 .sc-time{background:rgb(var(--accent-rgb) / .12);color:var(--cy)}
    .sc-3 .sc-time{background:rgb(var(--red-rgb) / .12);color:var(--red)}
    ...
    .data-table tr:hover td{background:rgb(var(--accent-rgb) / .03)}
    .data-table td:first-child{color:var(--tx);font-weight:600}
    .sig-strong{color:var(--red);font-weight:700}
    .sig-mod{color:var(--yellow);font-weight:600}
    ...
    .phase-list::before{content:'';position:absolute;left:7px;top:0;bottom:0;width:2px;background:linear-gradient(180deg,var(--accent),rgb(var(--accent-rgb) / .1))}
    ...
    .phase-dur{display:inline-block;background:rgb(var(--accent-rgb) / .1);color:var(--cy);font-size:.68rem;font-weight:700;padding:.1rem .5rem;border-radius:4px;margin-bottom:.3rem}
    ...
    .mistake-item{display:flex;align-items:flex-start;gap:1rem;padding:1rem 1.2rem;background:rgb(var(--red-rgb) / .04);border:1px solid rgb(var(--red-rgb) / .15);border-radius:10px;margin-bottom:.65rem;transition:border-color .2s}
    .mistake-item:hover{border-color:rgb(var(--red-rgb) / .3)}
    .mi-x{font-size:1rem;color:var(--red);flex-shrink:0;margin-top:2px}
      ...
      <div className="cspm-hero-wrap">
        ...
        <div className="img-wrap">
          <svg viewBox="0 0 960 380" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',display:'block',background:'linear-gradient(160deg,#071728 0%,#040f1b 100%)'}}>
            <defs>
              <pattern id="apat" width="22" height="22" patternUnits="userSpaceOnUse"><circle cx="11" cy="11" r=".7" fill="rgb(var(--accent-rgb) / 0.04)"/></pattern>
            </defs>
            ...
            {/* 6 trigger cards */}
            {[
              {n:'1', title:'Enterprise Customers', detail:'Asking for it in procurement', icon:'🏢', col:'var(--red)', x:30,  y:50},
              {n:'2', title:'International Markets', detail:'Europe, ME, Japan, Singapore', icon:'🌍', col:'var(--accent)', x:340, y:50},
              {n:'3', title:'Sensitive Data',        detail:'Financial, health, government', icon:'🔒', col:'var(--yellow)', x:650, y:50},
              {n:'4', title:'Investor Due Diligence',detail:'Series A/B security review',   icon:'💰', col:'var(--purple)', x:30,  y:188},
              {n:'5', title:'DPDP Act Compliance',  detail:'Indian data privacy law',       icon:'🇮🇳', col:'var(--accent)', x:340, y:188},
              {n:'6', title:'Prior Security Incident',detail:'Breach, ransomware, finding', icon:'🚨', col:'var(--red)', x:650, y:188},
            ].map(({n, title, detail, icon, col, x, y}) => (
              <g key={n}>
                <rect x={x} y={y} width="272" height="112" rx="12" fill="var(--bg-secondary)" stroke={`${col}28`} strokeWidth="1.5"/>
            ...
            {[
              {range:'0–1 YES', label:'Build basics first', col:'var(--text-secondary)', x:80},
              {range:'2–3 YES', label:'Begin scoping in 6 months', col:'var(--yellow)', x:290},
              {range:'4–5 YES', label:'Urgent - start now', col:'var(--accent)', x:530},
              {range:'6 YES',   label:'Business-critical', col:'var(--red)', x:760},
