...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
    ...
    .sd-beginner{background:rgb(var(--green-rgb) / .12);color:var(--green)}
    .sd-intermediate{background:rgb(var(--yellow-rgb) / .12);color:var(--yellow)}
    .sd-advanced{background:rgb(var(--red-rgb) / .12);color:var(--red)}
    ...
    .role-table tr:hover td{background:rgb(var(--accent-rgb) / .03)}
        ...
        <div className="img-wrap">
          <svg viewBox="0 0 960 420" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',display:'block',background:'linear-gradient(160deg,#071728 0%,#040f1b 100%)'}}>
            <defs>
              ...
              <linearGradient id="tg2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--green)"/><stop offset="100%" stopColor="var(--green)"/></linearGradient>
              <linearGradient id="tg3" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--purple)"/><stop offset="100%" stopColor="#6366f1"/></linearGradient>
              <pattern id="tpat" width="22" height="22" patternUnits="userSpaceOnUse"><circle cx="11" cy="11" r=".7" fill="rgb(var(--accent-rgb) / 0.04)"/></pattern>
            </defs>
            ...
            <rect x="16" y="52" width="220" height="352" rx="12" fill="var(--bg-secondary)" stroke="rgb(var(--accent-rgb) / 0.15)" strokeWidth="1"/>
            <text x="36" y="76" fill="rgba(200,214,229,0.7)" fontSize="10" fontWeight="700" fontFamily="sans-serif">EXERCISE TIMELINE</text>
            {[
              {time:'T-2 weeks', label:'Scenario selection & prep', col:'var(--green)', done:true},
              {time:'T-1 week',  label:'Participant briefing',      col:'var(--green)', done:true},
              {time:'T-1 day',   label:'Final logistics check',     col:'var(--green)', done:true},
              {time:'T+0:00',    label:'Scenario intro (15 min)',   col:'var(--accent)', done:false},
              {time:'T+0:15',    label:'Inject 1: Initial alert',   col:'var(--accent)', done:false},
              {time:'T+0:45',    label:'Inject 2: Escalation',      col:'var(--accent)', done:false},
              {time:'T+1:30',    label:'Inject 3: Complication',    col:'var(--yellow)', done:false},
              {time:'T+2:15',    label:'Hot debrief (45 min)',       col:'var(--purple)', done:false},
              {time:'T+2 wks',   label:'After-action report',       col:'var(--purple)', done:false},
            ...
            <line x1="250" y1="48" x2="250" y2="404" stroke="rgb(var(--accent-rgb) / 0.07)" strokeWidth="1"/>
            ...
            {/* Finding cards */}
            {[
              {gap:'No defined IR command structure',     owner:'CISO',    priority:'Critical', col:'var(--red)'},
              {gap:'Regulatory notification owner unclear',owner:'Legal',   priority:'Critical', col:'var(--red)'},
              {gap:'Backup recovery not tested in 18mo',  owner:'IT Ops',  priority:'High',     col:'var(--yellow)'},
              {gap:'Customer comms template missing',     owner:'Comms',   priority:'High',     col:'var(--yellow)'},
              {gap:'Executive escalation path unclear',   owner:'CEO Ofc', priority:'Medium',   col:'var(--accent)'},
            ].map(({gap,owner,priority,col},i)=>(
              <g key={i}>
                <rect x="262" y={84+i*60} width="400" height="48" rx="9" fill="var(--bg-secondary)" stroke={`${col}25`} strokeWidth="1"/>
            ...
            <text x="730" y="192" textAnchor="middle" fill="rgb(var(--accent-rgb) / 0.7)" fontSize="9" fontFamily="sans-serif">Pre-Exercise</text>
            ...
            <rect x="670" y="268" width="120" height="24" rx="8" fill="rgb(var(--green-rgb) / 0.08)" stroke="rgb(var(--green-rgb) / 0.25)" strokeWidth="1"/>
            <text x="730" y="284" textAnchor="middle" fill="var(--green)" fontSize="14" fontWeight="800" fontFamily="sans-serif">88% ↑</text>

            {/* Participant roles */}
            <text x="662" y="320" fill="rgba(200,214,229,0.6)" fontSize="9" fontWeight="600" fontFamily="sans-serif">Participants by Function</text>
            {[
              {role:'IT / Security',  icon:'🛡️', col:'var(--accent)'},
              {role:'Legal',          icon:'⚖️', col:'var(--purple)'},
              {role:'HR',             icon:'👥', col:'var(--yellow)'},
              {role:'Communications', icon:'📢', col:'var(--green)'},
              {role:'Leadership',     icon:'🏢', col:'var(--accent)'},
            ].map(({role,icon,col},i)=>(
              <g key={i}>
